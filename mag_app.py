...
# ══════════════════════════════════════════════════════════
#  НАЛАШТУВАННЯ СТОРІНКИ (Тема та Sidebar)
# ══════════════════════════════════════════════════════════
st.set_page_config(
...
# ══════════════════════════════════════════════════════════
#  МАТЕМАТИЧНА МОДЕЛЬ (оригінальна логіка)
# ══════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════
#  ЗБЕРЕЖЕННЯ СТАНУ ЗАСТОСУНКУ (Session State)
# ══════════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    if st.button("➕ Додати рядок до навчання", use_container_width=True):
        new_row = pd.DataFrame([{"X1": inp_x1, "X2": inp_x2, "Y": inp_y}])
        st.session_state.train_df = pd.concat([st.session_state.train_df, new_row], ignore_index=True)
    ...
    if uploaded_file is not None:
        try:
            ...
            if len(df_file.columns) >= 3:
                ...
                st.session_state.train_df = pd.concat([st.session_state.train_df, df_file], ignore_index=True)
    ...
    if st.button("🔮 Зробити прогноз", use_container_width=True):
        if st.session_state.coeffs is None:
            ...
            new_pred = pd.DataFrame([{"X1": inp_x1, "X2": inp_x2, "Y_predicted": round(y_pred_val, 4)}])
            st.session_state.pred_df = pd.concat([st.session_state.pred_df, new_pred], ignore_index=True)
...
# ══════════════════════════════════════════════════════════
#  ГОЛОВНА ПАНЕЛЬ (Метрики та вкладки)
# ══════════════════════════════════════════════════════════

# Верхній рядок з метриками
...
# Рядок виведення поточних коефіцієнтів моделі
st.markdown("### 📋 Коефіцієнти моделі")
if st.session_state.coeffs is not None:
    c = st.session_state.coeffs
    st.code(f"F₁={c[0]:.4f}   F₂={c[1]:.4f}   F₃={c[2]:.4f}   F₄={c[3]:.4f}   F₅={c[4]:.4f}   F₆={c[5]:.4f}", language="text")
else:
    st.info("Модель не навчена")

# Створення вкладок (Аналог Notebook у Tkinter)
tab_data, tab_plots = st.tabs(["📋 Дані застосунку", "📈 Графічний аналіз"])

# ── ВКЛАДКА 1: ДАНІ ТА ТАБЛИЦІ ─────────────────────────────
with tab_data:
    ...
    with col_t2:
        ...
        # Експорт прогнозів у Excel файл у пам'яті (без запису на локальний диск сервера)
...
with tab_plots:
    if st.session_state.coeffs is None:
        ...
        plot_type = st.radio("Оберіть графік аналізу:", ["Exp vs Predicted (Адекватність)", "Аналіз залишків", "Прогнозні значення"], horizontal=True)
        
        fig, ax = plt.subplots(figsize=(10, 5))
        
        if plot_type == "Exp vs Predicted (Адекватність)":
            ax.scatter(y, y_pred, color="#2EE59D", s=80, zorder=5, alpha=0.9, edgecolors="#5FFFC0", label="Точки")
            ...
            ax.scatter(y_pred, residuals, color="#2EE59D", s=80, alpha=0.9, edgecolors="#5FFFC0")
            ...
            if st.session_state.pred_df.empty:
                st.warning("Немає точок прогнозів для відображення кривих. Будь ласка, зробіть кілька прогнозів у сайдбарі.")
            else:
                df_p = st.session_state.pred_df.sort_values("X2")
                palette = ["#2EE59D", "#A3FF6F", "#5BC8FF", "#FFD166", "#FF5C7A"]
                
                for ci, x1_val in enumerate(sorted(df_p["X1"].unique())):
                    sub = df_p[df_p["X1"] == x1_val]
                    color = palette[ci % len(palette)]
                    ax.plot(sub["X2"], sub["Y_predicted"], marker="o", color=color, linewidth=2.2, label=f"X₁={x1_val:.3f}")
                ax.set_xlabel("X₂")
